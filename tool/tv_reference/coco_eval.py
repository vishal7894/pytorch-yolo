...
class CocoEvaluator(object):
    def __init__(self, coco_gt, iou_types, bbox_fmt='yolo'):
...
def convert_to_xywh(boxes, fmt='yolo'):
    if fmt.lower() == 'voc':
        xmin, ymin, xmax, ymax = boxes.unbind(1)
        return torch.stack((xmin, ymin, xmax - xmin, ymax - ymin), dim=1)
    elif fmt.lower() == 'yolo':
        check = boxes.unbind(1)[0]
        xcen, ycen, w, h = check[:,0],check[:,1],check[:,2],check[:,3]
